...
impl<'a> Lexer<'a> {
    ...
    fn read_ident_or_keyword(&mut self, start_pos: usize) -> Token<'a> {
        ...
        match s {
            ...
            "loop" => Token::Loop,
            "for" => Token::For,
            "R_FM" => Token::Number(consts::READ_FM),
            "STDOUT" => Token::Number(consts::STDOUT),
            "STDERR" => Token::Number(consts::STDERR),
            "W_FM" => Token::Number(consts::WRITE_FM),
            "C_FM" => Token::Number(consts::CREATE_FM),
            "T_FM" => Token::Number(consts::TRUNCATE_FM),
            "in" => Token::In,
